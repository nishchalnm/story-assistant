...
export default function EditorPage() {
  ...
  async function fetchProject() {
    try {
      ...
      // handle both flat {id, title...} and wrapped {project: {...}}
  ...
  async function retryEnhance() {
    if (!enhanceFeedback.trim() || isRetrying) return;
    setIsRetrying(true);
    ...
    try {
      ...
      console.error('Retry enhance failed', e);
    } finally {
      setIsRetrying(false);
    }
  }
  ...
  return (
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', background: 'var(--bg)', overflow: 'hidden' }}>
      ...
      <div style={{
        ...
        <button onClick={() => setLeftOpen(o => !o)} title="Toggle scenes panel"
          style={{ background: 'none', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', fontSize: '14px', padding: '0 4px' }}>☰</button>

        <button onClick={() => setRightOpen(o => !o)} title="Toggle chat panel"
          style={{ background: 'none', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', fontSize: '14px', padding: '0 4px' }}>💬</button>
      </div>
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
        ...
        {/* Left panel */}
        {leftOpen && (
          <div style={{ width: '240px', borderRight: '1px solid var(--border)', overflowY: 'auto', flexShrink: 0, padding: '20px 0' }}>
            ...
            {approvedScenes.map(scene => (
              <div key={scene.id} style={{ padding: '14px 20px', borderBottom: '1px solid var(--border)' }}>
          ...
          {showEnhanceReview ? (
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                ...
                {showFeedbackInput ? (
                  <>
                    <input
                      ...
                      onChange={e => setEnhanceFeedback(e.target.value)}
                      onKeyDown={e => {
                        if (e.key === 'Enter') retryEnhance();
                        if (e.key === 'Escape') setShowFeedbackInput(false);
                      }}
                      placeholder={isScreenplay
                        ? 'e.g. tighten the action lines, more subtext in dialogue...'
                        : 'e.g. more internal, slower pacing, bleaker tone...'}
                      style={{
                    ...
                    />
                    <button
                      onClick={retryEnhance}
                      disabled={isRetrying || !enhanceFeedback.trim()}
                      style={{
                        background: 'var(--accent)', border: '1px solid var(--accent)', color: 'var(--bg)',
                        padding: '6px 20px', fontSize: '11px', letterSpacing: '2px',
                        cursor: isRetrying || !enhanceFeedback.trim() ? 'default' : 'pointer',
                        fontFamily: 'JetBrains Mono, monospace',
                        opacity: !enhanceFeedback.trim() ? 0.6 : 1,
                      }}
                    >
                      {isRetrying ? 'RETRYING...' : 'RETRY'}
                    </button>
                    <button
                      onClick={() => { setShowFeedbackInput(false); setEnhanceFeedback(''); }}
                      style={{
                  ...
                  <>
                    <button onClick={acceptEnhancement} style={{
                      background: 'var(--accent)', border: '1px solid var(--accent)', color: 'var(--bg)',
                      padding: '6px 20px', fontSize: '11px', letterSpacing: '2px', cursor: 'pointer',
                      fontFamily: 'JetBrains Mono, monospace',
                    }}>ACCEPT</button>
                    <button onClick={() => setShowFeedbackInput(true)} style={{
                      background: 'transparent', border: '1px solid var(--accent)', color: 'var(--accent)',
                      padding: '6px 20px', fontSize: '11px', letterSpacing: '2px', cursor: 'pointer',
                      fontFamily: 'JetBrains Mono, monospace',
                    }}>TRY AGAIN</button>
                    <button onClick={rejectEnhancement} style={{
                      background: 'transparent', border: '1px solid var(--border)', color: 'var(--text-muted)',
                      padding: '6px 20px', fontSize: '11px', letterSpacing: '2px', cursor: 'pointer',
                      fontFamily: 'JetBrains Mono, monospace',
                    }}>REJECT</button>
              ...
              <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
                <div style={{ flex: 1, borderRight: '1px solid var(--border)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                  ...
                  <div style={{
                    flex: 1, overflow: 'auto', padding: '8px 32px 40px',
                    fontSize: isScreenplay ? '14px' : '16px',
                    lineHeight: isScreenplay ? '1.7' : '1.9',
                    fontFamily: isScreenplay ? 'JetBrains Mono, monospace' : 'Playfair Display, serif',
            ...
            <>
              <textarea
                ...
                placeholder={isScreenplay
                  ? `Scene ${sceneNumber}. INT./EXT. LOCATION - DAY/NIGHT`
                  : `Scene ${sceneNumber}. Begin writing...`}
                style={{
                  flex: 1,
                  background: 'transparent',
                  border: 'none',
                  ...
                  resize: 'none',
                  color: 'var(--text)',
                  fontSize: isScreenplay ? '14px' : '17px',
                  lineHeight: isScreenplay ? '1.7' : '1.9',
                  fontFamily: isScreenplay ? 'JetBrains Mono, monospace' : 'Playfair Display, serif',
                  padding: '60px 80px',
              ...
              />
              <div style={{
                padding: '12px 80px', borderTop: '1px solid var(--border)',
                fontSize: '11px', color: 'var(--text-muted)', display: 'flex', gap: '24px',
              }}>
        ...
        {/* Right panel — chat */}
        {rightOpen && (
          <div style={{ width: '340px', borderLeft: '1px solid var(--border)', display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
            <div style={{ fontSize: '10px', letterSpacing: '3px', color: 'var(--text-muted)', padding: '20px 20px 12px', borderBottom: '1px solid var(--border)' }}>
              {isScreenplay ? 'SCRIPT COLLABORATOR' : 'STORY COLLABORATOR'}
            </div>
            <div style={{ flex: 1, overflowY: 'auto', padding: '16px' }}>
              {chatMessages.length === 0 && (
                <div style={{ fontSize: '13px', color: 'var(--text-muted)', lineHeight: 1.7 }}>
                  {isScreenplay ? (
                    <>
                      Ask anything about your screenplay.<br /><br />
                      <span style={{ color: 'var(--border)', fontSize: '12px' }}>
                        &ldquo;Where am I in the three-act structure?&rdquo;<br />
                        &ldquo;What&apos;s the subtext in this scene?&rdquo;<br />
                        &ldquo;What plot threads are unresolved?&rdquo;<br />
                        &ldquo;Suggest a visual way to show this.&rdquo;
                      </span>
                    </>
                  ) : (
                    <>
              ...
              {chatMessages.map((msg, i) => (
                <div key={i} style={{ marginBottom: '20px', animation: 'fadeIn 0.3s ease' }}>
                  <div style={{ fontSize: '10px', letterSpacing: '2px', color: msg.role === 'user' ? 'var(--accent)' : 'var(--text-muted)', marginBottom: '6px' }}>
                    {msg.role === 'user' ? 'YOU' : 'COLLABORATOR'}
                  </div>
                  <div style={{ fontSize: '13px', lineHeight: '1.7', color: msg.role === 'user' ? 'var(--text)' : 'var(--text-muted)', whiteSpace: 'pre-wrap' }}>
                    {msg.content}
                  </div>
                </div>
              ))}
              {chatLoading && (
                <div style={{ fontSize: '12px', color: 'var(--text-muted)', fontStyle: 'italic' }}>thinking...</div>
              )}
            ...
            <div style={{ borderTop: '1px solid var(--border)', padding: '16px' }}>
              <textarea
                ...
                style={{
                  ...
                  color: 'var(--text)', fontSize: '13px', fontFamily: 'JetBrains Mono, monospace',
              ...
              <button
                ...
                style={{
                  ...
                  fontFamily: 'JetBrains Mono, monospace', opacity: !chatInput.trim() ? 0.4 : 1,
                }}
              >
